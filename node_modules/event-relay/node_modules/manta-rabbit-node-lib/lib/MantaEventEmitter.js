...
MantaEventEmitter.prototype.envelope = function ( options, cb ) {
	var self = this;
	var usage = "usage: MantaEvent.envelope( ( PAYLOAD(must be a Message) | eventType + payload object) ACTOR [SOURCEIDENTITY] )";
	...
	if (! options.payload.encode) { // duck Message type
		if (! ( options.eventType && options.payload ) ) {
			...
			options.payload = this.schema_mgr.message( options.eventType, options.payload, true );
	...
	newMessage.eventGroup = this.schema_mgr.get('MantaEvent').eventGroup;
	if (cb) return cb(null, this.schema_mgr.message( "MantaEvent", newMessage, false, this ));
	return this.schema_mgr.message( "MantaEvent", newMessage, true, this );
};
