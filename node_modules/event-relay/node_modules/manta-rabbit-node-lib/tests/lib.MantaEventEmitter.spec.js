...
describe("the emitter" , function () {
	...
	it( "should call trigger a listen", function (done) {
		tester.rabbit.on('MantaEventTransitional', function() {
				done();
			} );
	...
	it( "should encode a full manta event", function () {
			...
			expect( clone ).toEqual( { context : 'nodejs Event Emitter library', hostname : 'dihnen2-sbx', payload : '{"dog":"woof"}', actor : { id : 'dog', type : 'canine' }, sourceIdentity : { id : 'dog', type : 'canine' }, eventType : 'ClaimActivity', eventGroup : 'testgroup' }  );
	} );
	...
	it( "finishes", function () {
			expect(1).toBe(1);
			tester.rabbit.allDone();
			});
} );


//eventMgr.emitter('MantaEventPersist', function (emitter) {
