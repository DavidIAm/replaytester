...
, function (error, rab) {
		...
		rab.once('suzyV', function(d) { expect(d).toEqual("value F"); pcb() });
			...
			, function (pcb) {
					rab.emit('OtherExchange', 'ARoutingKey', [ 'O', 'Ark', 1 ], { headers: { header: "value G" }, replyTo: 'suzyW' } );
					rab.on('waitforG', function(d) { expect(d).toEqual("value G"); pcb(); });
					rab.emit('waitforG', 'value G');
				}
			, function (pcb) {
					rab.emit('OtherExchange', 'AnotherKey', [ 'O', 'Ank', 1 ], { headers: { header: "value H" }, replyTo: 'suzyX' } );
		rab.once('suzyX', function(d) { expect(d).toEqual("value H"); pcb() });
				}
			, function (pcb) {
					rab.emit('OtherExchange', 'AnotherKey', [ 'O', 'Ank', 2 ], { headers: { header: "value I" }, replyTo: 'suzyY' } );
		rab.once('suzyY', function(d) { expect(d).toEqual("value I"); pcb() });
				}
			, function (pcb) {
					rab.emit('OtherExchange', 'BRoutingKey', [ 'O', 'Brk', 1 ], { headers: { header: "value J" }, replyTo: 'suzyZ' } );
					rab.on('waitforJ', function(d) { expect(d).toEqual("value J"); pcb(); });
					rab.emit('waitforJ', 'value J');
				}
			, function (pcb) {
					rab.emit('OtherExchange', 'BnotherKey', [ 'O', 'Bnk', 1 ], { headers: { header: "value K" }, replyTo: 'suzyO' } );
		rab.once('suzyO', function(d) { expect(d).toEqual("value K"); pcb() });
				}
			, function (pcb) {
					rab.emit('OtherExchange', 'BnotherKey', [ 'O', 'Bnk', 2 ], { headers: { header: "value L" }, replyTo: 'suzyP' } );
