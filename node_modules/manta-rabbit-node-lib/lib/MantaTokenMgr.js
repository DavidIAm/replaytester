...
TokenMgr.prototype.validate = function (token, options, cb) {
	var error = [];
	if (! token) {
		error.push({ err: 'token was blank' })
		return cb(error);
		return error;
	}
	...
	if (token != withsecret)
		error.push({ err: 'token failed secret check', mismatch: withsecret })
	var time = this.now(); // default now
	if (options.atTime) time = options.atTime; // otherwise when specced
	if (time < tokenfields.start )
		error.push({ err: 'token not yet valid', now: time, start: tokenfields.start })
	if (time > tokenfields.expire)
		error.push({ err: 'token has expired', now: time, expire: tokenfields.expire })
	if (options.forType && tokenfields.type != options.forType)
		error.push({ err: 'token is not for this type', tokenType: tokenfields.type, sourceType: options.forType })
	if (options.forWhat && tokenfields.entity != options.forWhat)
		error.push({ err: 'token is not for this id', tokenId: tokenfields.entity, sourceId: options.forWhat })
	if (options.byWho && tokenfields.authed != options.byWho)
		error.push({ err: 'token is not for this actor', tokenId: tokenfields.authed, actorId: options.byWho })
